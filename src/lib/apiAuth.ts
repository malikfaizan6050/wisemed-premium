...
export async function authenticateCRMUser(request:Request,allowService=false):Promise<CRMAuthResult> {
    ...
    const fail=(status:number,code:string,error:string):CRMAuthResult=>{
        ...
        console.warn("[CRM auth]",{ stage,status,code });
    ...
    try {
        ...
        if(error instanceof FirebaseAdminConfigurationError || ["app/invalid-credential","auth/invalid-credential","auth/insufficient-permission","app/invalid-app-options"].includes(String(sdkCode)))
