...
export default function RolesPage() {
    return <PermissionGuard permission="roles.read"><RolesContent/></PermissionGuard>;
}
...
function RolesContent() {
    ...
    return <main className="min-h-screen bg-slate-50 p-6 md:p-8">
        ...
        <Modal open={modalOpen} title={editing ? "Edit role" : "Create role"} onClose={()=>setModalOpen(false)} maxWidth="xl">
            ...
            <fieldset className="mt-5"><legend className="text-sm font-semibold text-slate-700">Permissions</legend><div className="mt-3 grid gap-2 sm:grid-cols-2">{CRM_PERMISSIONS.map((permission)=><label key={permission} className="flex items-center gap-3 rounded-xl border p-3 text-sm text-slate-700"><input type="checkbox" checked={form.permissions.includes(permission)} onChange={()=>togglePermission(permission)} className="h-4 w-4 accent-blue-600"/><span>{formatPermission(permission)}</span></label>)}</div></fieldset>
