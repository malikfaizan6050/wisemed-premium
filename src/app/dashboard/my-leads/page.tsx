...
export default function MyLeadsPage() {
    ...
    const owned = useMemo(()=>leads.filter((lead)=>lead.ownerId === uid),[leads,uid]);
    ...
    const loadUsers = async() => {
        ...
        const response = await authenticatedFetch("/api/users?active=true&limit=100");
        const result = await response.json();
        if(response.ok && Array.isArray(result.users)) setUsers(result.users);
    };
    ...
    const exportCsv = () => {
        const rows = owned.filter((lead)=>selected.includes(lead.id));
        if(!rows.length){ setFeedback("Select at least one lead to export.");return; }
        ...
        const url = URL.createObjectURL(new Blob([csv],{ type:"text/csv;charset=utf-8" }));const anchor = document.createElement("a");anchor.href=url;anchor.download="my-leads.csv";anchor.click();URL.revokeObjectURL(url);
    };
